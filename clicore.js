...
program.command('getWalletBalance <walletId>')
	...
	.action(async (walletId) => console.absolute(await biot.getWalletBalance(walletId)));
...
program.command('listCorrespondents')
	...
	.action(async () => console.absolute(await biot.listCorrespondents()));
...
program.command('initWS')
	...
	.action((cmd) => {
		...
		const net = require('biot-wsocket/core');
		net.createServer(stateSequence,
			{host, port});
...
program.command('*')
	.action(() => {
		console.absolute('--------------------------');
		console.absolute('Command not found. Please use: biot-cli --help');
	});
